...
export const actions = {
    async default({cookies, request, params: {id}}) {
        ...
        if ( url === undefined && content === undefined) return fail(StatusCodes.BAD_REQUEST, {mediaType, empty: true})

        const sid = cookies.get('sid');
        if (!sid) return fail(StatusCodes.UNAUTHORIZED, {mediaType, url, content, authFail: true})

        const user = await getUserFromSession(sid);
        if (user === false) return fail(StatusCodes.UNAUTHORIZED, {mediaType, url, content, authFail: true})
        if (user.permission < Permission.RESEARCHER) return fail(StatusCodes.FORBIDDEN, {mediaType, url, content, noPerms: true})
        ...
        try {
            ...
            if (typeof result === 'object') return redirect(StatusCodes.MOVED_TEMPORARILY, `/earthquake/${id}`)
